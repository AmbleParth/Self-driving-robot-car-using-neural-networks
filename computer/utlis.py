...
def getName(filePath):
    ...
    return myImagePath
# Example usage:
#file_path = 'C:\\Users\\SARTHAK AMBLE\\Desktop\\Project\\DataCollected'
#file_name = getName(file_path)
#print(file_name)  # This will print 'Image_1698499192.jpg'


def importDataInfo(path):
    columns = ['Center', 'Steering']
    data = pd.DataFrame()  # Create an empty DataFrame to store the data
    for log_file in ['log_0.csv','log_1.csv']:
        log_data = pd.read_csv(os.path.join(path, log_file), names=columns)
        log_data['Center'] = log_data['Center'].apply(getName)
...
def dataGen(imagesPath, steeringList, batchSize, trainFlag):
    while True:
        ...
        for i in range(batchSize):
            ...
            # Debugging: Print the file path before attempting to open it
            #print("Attempting to open:", imagesPath[index])
            if trainFlag:
